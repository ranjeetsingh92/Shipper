...
@interface RSBookedViewController ()
{
...
@implementation RSBookedViewController
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    if (self.dataArray.count == 0)
    {
        ...
        NSString *userUpdate = [NSString stringWithFormat:@"{\"SubscriptionId\":%@,\"CreatedBy\":\"%@\",\"Status\":\"%@\",\"pageIndex\":%d,\"PageSize\":%d,\"TruckRequestStatus\":%@,\"IsAll\":%d}",[logDict objectForKey:@"subscriptionId"],[logDict objectForKey:@"emailId"],@"6",pageIndx,pageSiz,@"0" ,false];
        NSLog(@"%@",userUpdate);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.dataArray.count != 0) {
        if(indexPath.row <[self.dataArray count]){
            ...
            [cell.bidNowBtn addTarget:self action:@selector(btnSeePriceDetailsPressed:) forControlEvents:UIControlEventTouchUpInside];
            cell.bidNowBtn.layer.cornerRadius = 10.0;
            
            [cell.btnCancelLoad addTarget:self action:@selector(btnCancelLoadPressed:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)btnCancelLoadPressed:(UIButton *)sender
{
    
}
-(void)btnSeePriceDetailsPressed:(UIButton *)sender

{
    ...
    RSBookedBiddingDetailsViewController *biddingDetails = [self.storyboard instantiateViewControllerWithIdentifier:@"RSBookedBiddingDetailsViewController"];
