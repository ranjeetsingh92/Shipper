...
@implementation RSRoleViewController
...
- (void)viewDidLoad {
    ...
    [SVProgressHUD show];
    [self performSelector:@selector(getAllRole) withObject:nil afterDelay:0];
    [NSTimer scheduledTimerWithTimeInterval:30 target:self selector:@selector(getAllRole) userInfo:nil repeats:YES];
...
-(void)getAllRole
{
    ...
    NSString *completeLoginUrlWithSpace = [NSString stringWithFormat:@"%@%@",DOMAIN_URL,GET_ALL_ROLE];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *error)
     {
         ...
         if (data != nil)
         {
             ...
             self.dataArray = [[NSMutableArray alloc]initWithArray:[rsJson valueForKey:@"role"]];
...
//    NSLog(@"the string  is = %@",dataString);02
//    
//    NSData *mData = [dataString dataUsingEncoding:NSUTF8StringEncoding];
//    NSArray* dict = [[NSJSONSerialization JSONObjectWithData:mData options:0 error:nil] valueForKey:@"role"] ;
...
{
    RSAddNewRoleViewController *addRole = [self.storyboard instantiateViewControllerWithIdentifier:@"RSAddNewRoleViewController"];
...
-(void)loadDataDelayed
{
   ...
   // array = (NSMutableArray *)[Utils getAllRole:pageIndx pageSize:pageSiz];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *error)
     {
         if (data != nil)
         {
             ...
             NSMutableArray *array = [skillDict objectForKey:@"role"] ;
...
-(void)editBtnPressed:(UIButton *)sender
{
    ...
    RSAddNewRoleViewController *newBranch = [self.storyboard instantiateViewControllerWithIdentifier:@"RSAddNewRoleViewController"];
...
-(void)deleteBtnPressed:(UIButton *)sender
{
    ...
    NSString *completeLoginUrlWithSpace = [NSString stringWithFormat:@"%@%@",DOMAIN_URL,DELETE_ROLE];
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:completeLoginUrlWithSpace]];
