...
@implementation RSNewViewController
...
- (void)viewDidLoad {
    ...
    [SVProgressHUD show];
    [self performSelector:@selector(getNewLoad) withObject:nil afterDelay:0.0];
     [NSTimer scheduledTimerWithTimeInterval:30 target:self selector:@selector(getNewLoad) userInfo:nil repeats:YES];
...
-(void)getNewLoad
{
    ...
    if (self.dataArray.count == 0)
    {
        ...
        NSString *userUpdate = [NSString stringWithFormat:@"{\"SubscriptionId\":%@,\"CreatedBy\":\"%@\",\"Status\":\"%@\",\"pageIndex\":%d,\"PageSize\":%d,\"TruckRequestStatus\":%@,\"IsAll\":%d}",[logDict objectForKey:@"subscriptionId"],[logDict objectForKey:@"emailId"],@"5",pageIndx,pageSiz,@"0",false];
        NSLog(@"%@",userUpdate);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.dataArray.count != 0) {
        if(indexPath.row <[self.dataArray count]){
            ...
            if ([[[self.dataArray objectAtIndex:indexPath.row] valueForKey:@"toPickUpLocation"] count] == 0)
            {
...
-(void)goToDeleteVc:(UIButton *)sender
{
    
}
//-(void)goToFindTruckVc:(UIButton *)sender
//{
//    CGPoint buttonPosition = [sender convertPoint:CGPointZero toView:_rTableView];
//    NSIndexPath *aIndexPath = [_rTableView indexPathForRowAtPoint:buttonPosition];
//    
