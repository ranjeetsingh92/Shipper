...
@implementation RSRegionViewController
...
- (void)viewDidLoad {
    ...
    [SVProgressHUD show];
    [self performSelector:@selector(getAllRegion) withObject:nil afterDelay:0];
    [NSTimer scheduledTimerWithTimeInterval:30 target:self selector:@selector(getAllRegion) userInfo:nil repeats:YES];
...
-(void)getAllRegion
{
    ...
    NSString *completeLoginUrlWithSpace = [NSString stringWithFormat:@"%@%@",DOMAIN_URL,GET_ALL_REGIONS];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *error)
     {
         ...
         if (data != nil)
         {
             ...
             self.dataArray = [[NSMutableArray alloc]initWithArray:[rsJson valueForKey:@"region"]];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *error)
     {
         if (data != nil)
         {
             ...
             NSMutableArray *array = [skillDict objectForKey:@"region"] ;
...
-(void)editBtnPressed:(UIButton *)sender
{
    ...
    RSAddNewRegionViewController *addRegion = [self.storyboard instantiateViewControllerWithIdentifier:@"RSAddNewRegionViewController"];
    addRegion.receiveDataDict = [self.dataArray objectAtIndex:aIndexPath.row];
    [self.navigationController pushViewController:addRegion animated:YES];
}
-(void)deleteBtnPressed:(UIButton *)sender
{
    ...
    NSString *completeLoginUrlWithSpace = [NSString stringWithFormat:@"%@%@",DOMAIN_URL,DELETE_REGION];
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:completeLoginUrlWithSpace]];
