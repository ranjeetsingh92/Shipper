...
#import "RSDepartmentViewController.h"
#import "RSRegionTableViewCell.h"
#import "RSAddNewDepartmentViewController.h"

@interface RSDepartmentViewController ()
{
    int pageIndx ;
    int pageSiz ;
...
@implementation RSDepartmentViewController
...
- (void)viewDidLoad {
    ...
    [SVProgressHUD show];
    [self performSelector:@selector(getDepartments) withObject:nil afterDelay:0];
    [NSTimer scheduledTimerWithTimeInterval:30 target:self selector:@selector(getDepartments) userInfo:nil repeats:YES];
...
-(void)getDepartments
{
    ...
    NSString *completeLoginUrlWithSpace = [NSString stringWithFormat:@"%@%@",DOMAIN_URL,GET_ALL_DEPARTMENT];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *error)
     {
         ...
         if (data != nil)
         {
             ...
             self.dataArray = [[NSMutableArray alloc]initWithArray:[rsJson valueForKey:@"department"]];
...
{
    RSAddNewDepartmentViewController *addNewDepartment = [self.storyboard instantiateViewControllerWithIdentifier:@"RSAddNewDepartmentViewController"];
    [self.navigationController pushViewController:addNewDepartment animated:YES];
}
...
-(void)loadDataDelayed
{
   ...
   // array = (NSMutableArray *)[Utils getAllDepartment:pageIndx pageSize:pageSiz];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *error)
     {
         if (data != nil)
         {
             ...
             NSMutableArray *array = [skillDict objectForKey:@"department"] ;
...
-(void)editBtnPressed:(UIButton *)sender
{
    ...
    RSAddNewDepartmentViewController *newBranch = [self.storyboard instantiateViewControllerWithIdentifier:@"RSAddNewDepartmentViewController"];
...
-(void)deleteBtnPressed:(UIButton *)sender
{
    ...
    NSString *completeLoginUrlWithSpace = [NSString stringWithFormat:@"%@%@",DOMAIN_URL,DELETE_DEPARTMENT];
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:completeLoginUrlWithSpace]];
