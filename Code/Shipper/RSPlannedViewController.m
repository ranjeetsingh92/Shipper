...
@interface RSPlannedViewController ()
{
    int pageIndx ;
    int pageSiz ;
...
@implementation RSPlannedViewController
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    if (self.dataArray.count == 0)
    {
        ...
        NSString *userUpdate = [NSString stringWithFormat:@"{\"SubscriptionId\":%@,\"CreatedBy\":\"%@\",\"Status\":\"%@\",\"pageIndex\":%d,\"PageSize\":%d,\"TruckRequestStatus\":%@,\"IsAll\":%d}",[logDict objectForKey:@"subscriptionId"],[logDict objectForKey:@"emailId"],@"7",pageIndx,pageSiz,@"0",false];
        NSLog(@"%@",userUpdate);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.dataArray.count != 0) {
        if(indexPath.row <[self.dataArray count]){
            ...
            [cell.bidNowBtn addTarget:self action:@selector(seePriceDetails:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)seePriceDetails:(UIButton *)sender
{
    
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    RSPlannedDetailsViewController *biddingDetails = [self.storyboard instantiateViewControllerWithIdentifier:@"RSPlannedDetailsViewController"];
    biddingDetails.receiveDataDict = [self.dataArray objectAtIndex:indexPath.row];
...
#pragma mark UserDefined Method for generating data which are show in Table
