...
@interface RSBiddingViewController ()
{
...
@implementation RSBiddingViewController
...
- (void)viewDidLoad
{
    ...
    [SVProgressHUD show];
    [self performSelector:@selector(getBiddingData) withObject:nil afterDelay:0.0];
    [NSTimer scheduledTimerWithTimeInterval:30 target:self selector:@selector(getBiddingData) userInfo:nil repeats:YES];
}
-(void)getBiddingData
{
    ...
    if (self.dataArray.count == 0)
    {
        ...
        if (self.dataArray.count == 0)
        {
            ...
            NSString *userUpdate = [NSString stringWithFormat:@"{\"SubscriptionId\":%@,\"CreatedBy\":\"%@\",\"Status\":\"%@\",\"pageIndex\":%d,\"PageSize\":%d,\"TruckRequestStatus\":%@,\"IsAll\":%d}",[logDict objectForKey:@"subscriptionId"],[logDict objectForKey:@"emailId"],@"4",pageIndx,pageSiz,@"0",false];
            NSLog(@"%@",userUpdate);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    RSBiddingDetailViewController *biddingDetails = [self.storyboard instantiateViewControllerWithIdentifier:@"RSBiddingDetailViewController"];
