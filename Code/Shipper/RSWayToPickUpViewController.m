...
#import "RSWayToPickUpViewController.h"
#import "RSWay2PickTableViewCell.h"
#import "RSDTrackViewController.h"
#import "RSTrackHistoryViewController.h"

@interface RSWayToPickUpViewController ()
{
    int pageIndx ;
    int pageSiz ;
    RSWay2PickTableViewCell *cell;
}

@end

@implementation RSWayToPickUpViewController
...
-(void)getAllWayToPickupLocationLoad
{
    ...
    NSString *userUpdate = [NSString stringWithFormat:@"{\"SubscriptionId\":%@,\"pageIndex\":%d,\"pageSize\":%d,\"Status\":%@,\"SubscriptionType\":\"%@\",\"WorkingSubscriptionType\":\"%@\"}",[[Utils getLogingDict] objectForKey:@"subscriptionId"],pageIndx,pageSiz,@"7",[[Utils getLogingDict] objectForKey:@"subscriptionType"],@""];
    NSLog(@"%@",userUpdate);
    
    NSString *completeLoginUrlWithSpace = [NSString stringWithFormat:@"%@%@",DOMAIN_URL,WAYTOPICKUPTRACK];
...
-(void)valueChanged:(UIButton *)sender
{
    ...
    RSDTrackViewController *showDetails = [self.storyboard instantiateViewControllerWithIdentifier:@"RSDTrackViewController"];
    showDetails.shipmentId = [[self.dataArray objectAtIndex:aIndexPath.row] valueForKey:@"id"];
...
-(void)detailBtnPressed:(UIButton *)sender
{
    ...
    RSTrackHistoryViewController *showDetails = [self.storyboard instantiateViewControllerWithIdentifier:@"RSTrackHistoryViewController"];
    showDetails.shipmentId = [[self.dataArray objectAtIndex:aIndexPath.row] valueForKey:@"id"];
    [self.navigationController pushViewController:showDetails animated:YES];
